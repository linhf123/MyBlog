...
export default function Navbar() {
  ...
  const [previousTimeString, setPreviousTimeString] = useState('');
  ...
  // 渲染时间数字，检测变化并添加掉落动画
  const renderTimeWithDropAnimation = () => {
    const currentTimeString = formatTime(currentTime);
    const timeChars = currentTimeString.split('');
    const prevChars = previousTimeString.split('');
    
    // 更新前一次的时间字符串
    if (previousTimeString !== currentTimeString) {
      setPreviousTimeString(currentTimeString);
    }
    
    return timeChars.map((char, index) => {
      // 检查当前位置的字符是否发生了变化
      const hasChanged = prevChars[index] !== char && previousTimeString !== '';
      ...
      return (
        ...
        >
          {/* 当前数字 */}
          <span
            key={`current-${index}-${char}-${hasChanged ? Date.now() : 'static'}`}
            className={`absolute inset-0 flex items-center justify-center transition-all duration-300 ${
              hasChanged ? 'animate-digit-drop-in' : ''
            }`}
          >
            {char}
          </span>
          ...
          {/* 前一个数字（掉落时显示） */}
          {hasChanged && prevChars[index] && (
            <span
              key={`previous-${index}-${prevChars[index]}-${Date.now()}`}
              className="absolute inset-0 flex items-center justify-center animate-digit-drop-out"
            >
              {prevChars[index]}
            </span>
          )}
    ...
    <nav className="sticky top-0 z-50 bg-gradient-to-r from-indigo-600/95 via-purple-600/95 to-blue-600/95 backdrop-blur-xl shadow-xl border-b border-white/20">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          {/* Right Section with Time and Auth */}
          <div className="flex items-center space-x-6">
            ...
            <div className="flex items-center space-x-2 bg-white/10 backdrop-blur-sm rounded-xl px-4 py-2 border border-white/20">
              ...
              <div className="text-white font-mono text-lg tracking-wider flex">
                {renderTimeWithDropAnimation()}
              </div>
            </div>
