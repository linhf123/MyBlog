...
export default function PostCard({ post, index = 0 }: PostCardProps) {
  return (
    <article className="bg-white/90 rounded-2xl shadow-lg hover:shadow-xl transition-shadow duration-200 border border-white/30 p-8 group relative will-change-transform">
      {/* 文章头部 */}
      <div className="flex items-start justify-between mb-6">
        <div className="flex-1">
          <Link href={`/posts/${post.id}`}>
            <h2 className="text-2xl lg:text-3xl font-bold text-gray-800 group-hover:text-indigo-600 transition-colors duration-200 leading-tight">
              {post.title}
            </h2>
          </Link>
      ...
      <div className="flex items-center justify-between pt-6 border-t border-gray-100">
        ...
        <Link 
          href={`/posts/${post.id}`}
          className="group/btn inline-flex items-center space-x-2 text-indigo-600 hover:text-indigo-800 font-medium text-sm transition-colors duration-200"
        >
          <span>阅读全文</span>
